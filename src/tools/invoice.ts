...
export const createInvoiceTool: Tool = {
    ...
    inputSchema: {
      type: "object",
      properties: {
        customer_id: { type: "string", description: "ID of the customer the invoice is for" },
        line_items: {
          type: "array",
          items: { type: "object" },
  ...
  export const listInvoicesTool: Tool = {
    ...
    inputSchema: {
      type: "object",
      properties: {
        params: { type: "object", description: "Query parameters (optional)", additionalProperties: true },
      },
      required: [],
    },
  };
  ...
  export const getInvoiceEmailTool: Tool = {
    ...
    description: "Get payment reminder email content for a specific invoice (organization_id is fixed)",
    inputSchema: {
      type: "object",
      properties: {
        invoice_id: { type: "string", description: "ID of the invoice to fetch email content for" }
      },
      required: ["invoice_id"]
    }
  };
  ...
  export const listInvoicePaymentsTool: Tool = {
    ...
    description: "List all payments made against a specific invoice (organization_id is fixed)",
    inputSchema: {
      type: "object",
      properties: {
        invoice_id: { type: "string", description: "Invoice ID for which to retrieve payments" },
      },
      required: ["invoice_id"],
    },
  };
  ...
  export const listCreditsAppliedTool: Tool = {
    ...
    description: "Get a list of credits applied to a specific invoice (organization_id is fixed)",
    inputSchema: {
      type: "object",
      properties: {
        invoice_id: { type: "string", description: "Invoice ID for which to retrieve applied credits" },
      },
      required: ["invoice_id"],
    },
  };
  ...
  export const generatePaymentLinkTool: Tool = {
    ...
    inputSchema: {
      type: "object",
      properties: {
        transaction_id: { type: "string", description: "Transaction ID (e.g., invoice ID)" },
        transaction_type: { 
          type: "string", 
